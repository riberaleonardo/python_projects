...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for city_name, (lat, lon) in CITIES.items():
            ...
            try:
                ...
                if not current_weather:
                    self.stderr.write(f"No current weather for {city_name}")
                    skipped_count += 1
                    continue
                ...
                api_time = timezone.make_aware(api_time)
                ...
                self.stderr.write(f"Timeout for {city_name}")
                skipped_count += 1
        ...
        self.stdout.write(self.style.SUCCESS(f"Created {current_created} weather records"))
        self.stdout.write(self.style.SUCCESS(f"Updated {current_updated} weather records"))
